...
DWORD WINAPI CSocketManager::ServerAcceptThread(CSocketManager* lpSocketManager) // OK
{
	...
	while(true)
	{
		...
		int poisd = WSARecv(socket, &lpObj->PerSocketContext->IoRecvContext.wsabuf, 1, &RecvSize, &Flags, &lpObj->PerSocketContext->IoRecvContext.overlapped, 0);
		if(poisd == SOCKET_ERROR)
		{
			if(WSAGetLastError() != WSA_IO_PENDING)
			{
				static int asd = WSAGetLastError();
				gLog.Output(LOG_CONNECT,"[SocketManager] WSARecv() failed with error: %d", asd);
