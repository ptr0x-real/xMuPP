...
void ProtocolCore(BYTE head,BYTE* lpMsg,int size,int aIndex,int encrypt,int serial) // OK
{
	...
	if (packet_subcode == -1)
	{
		LogAdd(LOG_BLUE, "New packet recvd. Head: 0x%X. Size: %d", head, size);
	}
	else
	{
		LogAdd(LOG_BLUE, "New packet recvd. Head: 0x%X. Subcode: 0x%X. Size: %d", head, packet_subcode, size);
	}
